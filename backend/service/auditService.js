...
exports.logAudit = async (userEmail, userRole, action, entityType, details) => {
  try {
    await Connection(
      `
      INSERT INTO audit_logs (user, userRole, action, entityType, details)
      VALUES (?, ?, ?, ?, ?)
      `,
      [userEmail, userRole, action, entityType, details]
    );
  } catch (err) {
